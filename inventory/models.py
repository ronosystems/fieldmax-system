...
class Product(models.Model):
    """
    Represents inventory items with clear separation between:
    - PRODUCT CODE (SKU): Unique identifier for the product type/variant
    - SERIAL NUMBER: Unique per physical item (for single items)
    - IMEI NUMBER: Unique for phones (15-digit)
    - BARCODE: Scannable representation of product data
    """
    ...
    product_code = models.CharField(
        ...
        help_text="Unique product identifier (F00101, F00102, etc.)"
    )
    ...
    sku_value = models.CharField(
        ...
        null=True,
        db_index=True,
        verbose_name="SKU Value (Legacy)",
        help_text="Legacy field - Use product_code instead"
    )
    ...
    barcode = models.CharField(
        ...
        help_text="Scannable barcode for product data"
    )
    ...
    specifications = models.JSONField(
        ...
        help_text="RAM, storage, color, screen size, etc. Example: {'ram': '8GB', 'storage': '256GB', 'color': 'Black'}"
    )
    ...
    best_price = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name="Best/Retail Price"
    )
    ...
    # ============================================
    # THEFT / LOSS TRACKING (Consolidated)
    # ============================================
    is_stolen_or_lost = models.BooleanField(
        default=False,
        verbose_name="Is Stolen or Lost",
    ...
    police_report_number = models.CharField(
        ...
        blank=True, 
        null=True,
        verbose_name="Police Report Number",
    ...
    class Meta:
        ...
        unique_together = [
            ...
            ['category', 'imei_number'],
        ]
    
    def save(self, *args, **kwargs):
        """Auto-generate codes and validate"""
        
        # Store who modified this
        if 'modified_by' in kwargs:
            self.last_modified_by = kwargs.pop('modified_by')
        
        # Generate product code if not exists (FORMAT: F00101)
        if not self.product_code:
            self.product_code = self._generate_product_code()
        
        # Auto-generate name from brand/model if blank
        ...
        # Auto-update status
        if self.status not in ['stolen', 'writeoff', 'recalled']:
            self._update_status()
    ...
    def _generate_product_code(self):
        ...
        Format: F + 5-digit sequential number
        Starting from F00101
        Examples: F00101, F00102, F00103
        """
        try:
            # Get the highest existing product code
            max_code = Product.objects.filter(
                product_code__regex=r'^F\d{5}$'
            ).aggregate(Max('product_code'))['product_code__max']
            
            if max_code and max_code.startswith('F'):
                try:
                    # Extract number from last code (F00101 -> 101)
                    last_number = int(max_code[1:])
                    new_number = last_number + 1
                except (ValueError, IndexError):
                    new_number = 101
            else:
                # No existing products, start from 101
                new_number = 101
            
            return f"F{str(new_number).zfill(5)}"
            
        except Exception as e:
            logger.error(f"Error generating product code: {e}")
            # Fallback using timestamp
            import time
            return f"F{str(int(time.time()))[-5:]}"
    ...
    def _generate_barcode(self):
        ...
        base = f"{self.product_code}{time.time()}{random.randint(1000, 9999)}"
    ...
    def clean(self):
        ...
        # For single items, require either serial_number or imei_number
        if self.category.is_single_item:
            if self.status not in ['sold', 'stolen', 'writeoff']:
                if not self.serial_number and not self.imei_number:
                    raise ValidationError(
                        "Single items must have either a Serial Number or IMEI Number"
                    )
            
            if self.imei_number and len(self.imei_number) != 15:
    ...
    def primary_identifier(self):
        """Return the primary unique identifier for this product"""
        if self.imei_number:
    ...
    def get_absolute_url(self):
        """Returns the URL to view this product's detail page"""
    ...
    def __str__(self):
        ...
        if self.category and self.category.is_single_item:
            return f"{self.display_name} - {identifier} ({self.product_code}){stolen_indicator}"
        
        return f"{self.display_name} ({self.product_code}) - Qty: {self.quantity}{stolen_indicator}"
...
class StockEntry(models.Model):
    ...
    def clean(self):
        ...
        if self.product.category and self.product.category.is_single_item:
            if self.entry_type in ['purchase', 'reversal', 'return'] and abs(self.quantity) != 1:
...
def validate_stock_entry(sender, instance, **kwargs):
    ...
    try:
        ...
        if instance.product.category and instance.product.category.is_single_item:
            if instance.entry_type in ['purchase', 'reversal', 'return'] and abs(instance.quantity) != 1:
