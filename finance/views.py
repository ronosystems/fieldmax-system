...
from .models import (
    ...
    IncomeTransaction, PurchaseTransaction, ProfitTransaction, AccountTransaction, 
)
from sales.models import Sale, SaleItem
...
# ============================================
# FINANCE DASHBOARD - WITH PERIOD FILTERING
# ============================================
...
def financial_overview(request):
    ...
    from .models import SavingsAccount, InjectionAccount, NetAccount, InventoryAsset
...
@login_required
def inventory_asset_view(request):
    """View inventory asset value"""
    from .models import InventoryAsset
    
    inventory_asset = InventoryAsset.get_account()
    transactions = inventory_asset.transactions.all()[:50]
    ...
    for product in Product.objects.filter(is_active=True, is_discontinued=False):
        ...
        total_units += stock
        current_inventory_value += stock * product.buying_price
    ...
    context = {
        ...
        'sync_match': inventory_asset.current_value == current_inventory_value,
    }
    
    return render(request, 'finance/inventory_asset.html', context)
...
def injection_account_detail(request):
    ...
    # Get staff list (needed for template consistency)
    staff_list = User.objects.filter(is_active=True).order_by('first_name', 'username')
    ...
    context = {
        ...
        # Pass empty lists for template compatibility (injection account doesn't need staff/shop)
...
def net_account_detail(request):
    ...
    context = {
        ...
        'deductions_breakdown': {
            'Inventory_Purchases': net.total_inventory_purchases,
            'Salaries': net.total_salaries,
            'Operational_Expenses': net.total_operational_expenses,
        },
        # Add these for the modals
        'staff_list': staff_list,
        'shop_list': shop_list,
        'months': months,
        'year_range': year_range,
...
@login_required
def savings_add_expense(request):
    """Add expense from Savings Account (optional)"""
    from .models import SavingsAccount, SavingsTransaction, FinancialTransaction
    from decimal import Decimal
    ...
    if request.method == 'POST':
        ...
        if amount <= 0:
            messages.error(request, 'Amount must be greater than 0')
            return redirect('finance:savings_account')
        
        savings = SavingsAccount.get_account()
        
        if savings.balance < amount:
            messages.error(request, f'Insufficient savings balance. Available: KES {savings.balance:,.2f}')
            return redirect('finance:savings_account')
        
        with transaction.atomic():
            # Deduct from Savings
            savings.balance -= amount
            savings.total_profits_earned -= amount  # Reduce total profits earned
            savings.save()
            
            # Create SavingsTransaction record
            ...
            FinancialTransaction.objects.create(
                ...
                category='operational',
                amount=amount,
                description=f"Savings Expense - {expense_type}: {description}",
                payment_method=payment_method,
                payment_reference=payment_reference,
                recipient_name=recipient_name,
                created_by=request.user,
                notes=f"Expense paid from Savings Account"
            )
            
            # Also deduct from Net Account? (Optional - if you want both)
            # If you want to also track that cash left the business, 
            # you can also deduct from Net or Bank/Cash account
            if payment_method == 'cash':
                cash_account, _ = CashAccount.objects.get_or_create(id=1)
                cash_account.update_balance(amount, 'expense', request.user)
            elif payment_method == 'bank':
                bank_account, _ = BankAccount.objects.get_or_create(id=1)
                bank_account.update_balance(amount, 'expense', request.user)
        
        messages.success(request, f'✅ {expense_type.upper()} expense of KES {amount:,.2f} deducted from Savings Account')
        return redirect('finance:savings_account')
    ...
    return render(request, 'finance/savings_add_expense.html', context)
...
def savings_account_detail(request):
    ...
    context = {
        'account': savings,
        'savings': savings,  # Pass savings for balance display
        'transactions': page_obj,
