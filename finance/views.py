...
@csrf_exempt
def mpesa_callback(request):
    """Handle Kopo Kopo webhook callbacks - LINK TO SALE"""
    if request.method != 'POST':
        return HttpResponse("Method not allowed", status=405)
    ...
    try:
        ...
        # Get the reference from metadata
        metadata = data.get('metadata', {})
        account_reference = metadata.get('reference', '')
        
        logger.info(f"Account Reference from metadata: {account_reference}")
        
        # Extract payment ID
        payment_id = resource_data.get('id') or data.get('data', {}).get('id')
        
        # Extract status
        status = resource_data.get('status')
        amount = resource_data.get('amount')
        receipt = resource_data.get('reference')
        phone_number = resource_data.get('sender_phone_number')
        
        # ✅ CRITICAL: Find the sale by account_reference
        sale = None
        ...
        if not created:
            ...
            transaction.sale = sale  # ✅ Link the sale
            transaction.mpesa_receipt_number = receipt
            transaction.callback_raw_data = data
            transaction.save()
        
        # ✅ If sale exists and payment is completed, update the sale
        if sale and status == 'Success':
